...
<body">
    ...
    <table style="width: 80vw; line-height: 5vh; margin-top: 5vh;">
        ...
        <t>
            ...
            $sql_patient_doctor = 'SELECT * FROM project.pd_relation where pd_relation.patientEmail = "'.$email_email_id.'"';
            $result_patient_doctor = mysqli_query($dbconn, $sql_patient_doctor);
            while ($row_patient_doctor = mysqli_fetch_assoc($result_patient_doctor)) {
            ?>
            <tr>
                <td><?php echo $row_patient_doctor['doctor_id']; ?> </td>
                <td><?php echo $row_patient_doctor['name']; ?> </td>
                <td><?php echo $row_patient_doctor['phone_no']; ?> </td>
                <td><?php echo $row_patient_doctor['email']; ?> </td>
                <td><?php echo $row_patient_doctor['Diagnosis']; ?> </td>
                <td><?php echo $row_patient_doctor['Prescription']; ?> </td>
                <td><?php echo $row_patient_doctor['PatientEmail']; ?> </td>
            </tr>
            <?php
            }
            ?>
