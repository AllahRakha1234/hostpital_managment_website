...
<body style='background: url("img/bgdoctor.jpg") no-repeat center center/cover;'>
    ...
    <?php
        ...
        if (isset($_POST['Submit'])) {
            ...
            $sql = 'SELECT * FROM project.patient where email = "' . $email . '" AND pass = "' . $password . '";';
            $result = mysqli_query($dbconn, $sql);
            $num = mysqli_num_rows($result);
            if ($num == 1) {
                header("Location: patientRights.php");
            } else {
        ...
        ?>
    <!-- Nurse Login Modal -->
    <div id="patient-container" class="adm-container">
        <div id="patient-login" class="adm-login">
            <form action="patientlogin.php" method="POST">
                <h3>Login Page</h3>
                Email: <br><input type="patient-email" name="patient-email" id="patient-email"><br>
                Password: <br><input type="text" name="patient-password" id="patient-password"><br>
                <input type="submit" name="Submit" id="patient-submit-btn" class="adm-submit-btn">
            </form>
        </div>
    </div>
    <!-- Footer -->
