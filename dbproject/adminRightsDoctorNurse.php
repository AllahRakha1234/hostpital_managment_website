...
<body>
    ...
    if (isset($_POST['update_second'])) {
        ...
        if ($doctor_id != "" && $nurse_id != "") {
            ...
            if ((mysqli_num_rows($result_doctor) != 0) && (mysqli_num_rows($result_nurse) != 0)) {
                // $sql = 'INSERT INTO doctor_nurse (doctor_id, nurse_id) VALUES (' . $doctor_id . ', ' . $nurse_id . ');';
                $sql = 'UPDATE project.doctor_nurse  SET doctor_id =' . $doctor_id . ', nurse_id = ' . $nurse_id . ' WHERE doctor_nurse.doctor_id = ' . $pre_doc_id . ' AND doctor_nurse.nurse_id = ' . $pre_nurs_id . '';
                $result = mysqli_query($dbconn, $sql);
                header("Location: adminRightsDoctorNurse.php");
            } else {
                echo '<script text = "Error">alert("DATA NOT FOUND")</script>';
    ...
    if (isset($_POST['delete'])) {
        ...
        $delete_id_nurse = $_POST["delete_nurse_id_no"];
        if ($delete_id_doc != "" && $delete_id_nurse != "") {
            $sql = 'Select * FROM doctor_nurse Where doctor_nurse.doctor_id =' . $delete_id_doc . ' AND doctor_nurse.nurse_id =' . $delete_id_nurse . ';';
            $result = mysqli_query($dbconn, $sql);
            $num = mysqli_num_rows($result);
            if ($num == 1) {
                $sql = 'DELETE FROM doctor_nurse where doctor_nurse.doctor_id =' . $delete_id_doc . ' AND 
            doctor_nurse.nurse_id =' . $delete_id_nurse . ';';
                $result = mysqli_query($dbconn, $sql);
                header("Location: adminRightsDoctorNurse.php");
            } else {
                echo '<script text = "Error">alert("DATA NOT FOUND")</script>';
        ...
        $pre_id_nurse = $_POST["update_nurse_id_no"];
        if ($pre_id_doctor != "" && $pre_id_nurse != "") {
            $_SESSION['pre_doctor_id'] = $pre_id_doctor;
            $_SESSION['pre_nurse_id'] = $pre_id_nurse;
            $sql = 'Select * FROM doctor_nurse where doctor_nurse.doctor_id =' . $pre_id_doctor . ' AND doctor_nurse.nurse_id = ' . $pre_id_nurse . ';';
            $result = mysqli_query($dbconn, $sql);
            $num = mysqli_num_rows($result);
            if ($num == 1) {
                ...
                <table style="width: 80vw; line-height: 5vh; line-width: 5vh">
                    ...
                    <t>
                        ...
                        // $sql = 'SELECT * FROM project.doctor_nurse where doctor.doctor_id =' . $update_id . ';';
                        $sql = 'SELECT * FROM doctor_nurse Where doctor_nurse.doctor_id =' . $pre_id_doctor . ' AND doctor_nurse.nurse_id =' . $pre_id_nurse . ';';
                        $result = mysqli_query($dbconn, $sql);
                        ...
                        ?>
                        <tr>
                            <form action="adminRightsDoctorNurse.php" method="POST">
                                <td><input type="text" name="doctor_id" class="input_width"></td>
                                <td><input type="text" name="nurse_id" class="input_width"></td>
    ...
    <div class="delete-edit-container">
        <div class="delete-data">
            <form action="adminRightsDoctorNurse.php" method="POST">
                ...
                <input type="text" name="delete_nurse_id_no" id="delete_id" placeholder="Enter nurse ID">
        ...
        <div class="edit-data">
            <form action="adminRightsDoctorNurse.php" method="POST">
                ...
                <input type="text" name="update_nurse_id_no" id="update_id_no" placeholder="Enter nurse ID">
    ...
    <table style="width: 80vw; line-height: 5vh">
        ...
        <t>
            ...
            <t />
            <?php
            $sql_ds = 'SELECT * FROM project.doctor_nurse';
            $result_ds = mysqli_query($dbconn, $sql_ds);
            while ($row_ds = mysqli_fetch_assoc($result_ds)) {
                ?>
                <tr>
                    <td><?php echo $row_ds['doctor_id']; ?> </td>
                    <td>
                        <?php echo $row_ds['nurse_id']; ?>
                    </td>
                </tr>
                <?php
            }
